...
외부 공개 프로젝트는 기본적으로 claim 전 상태를 표시한다. 실제 소유자가 claim을 요청하면 pending 상태로 남고, 현재 글 owner가 승인해야 프로젝트가 소유자 프로필로 이동한다. 요청자는 승인/거절 전 pending claim을 철회할 수 있다. 원 등록자는 submittedBy 기록으로 남는다. 소유자 요청이 있으면 삭제 또는 비공개 처리한다.
