...
VibeReview는 AI 코딩으로 만든 프로젝트를 가볍게 올리고, 개인 프로필에 모아두고, 프로젝트 글에서 댓글형 피드백을 받는 게시판형 서비스다.
...
단순 프로젝트 게시판에서 멈추지 않고, 반드시 피드백을 받는 프로젝트 작업장으로 발전시킨다. 핵심은 AI 첫 피드백 하네스, 사람 피드백 배정 큐, 피드백에 대한 평가, 리뷰어 평판, 기여 기반 자료 접근 권한이다.

자세한 방향은 [Feedback System Direction](./feedback-system.md)에 정리한다.

프로젝트 관리는 별도 업무툴이 아니라 피드백을 반영하고 다음 액션을 잃어버리지 않게 돕는 가벼운 작업 로그로 둔다. 자세한 방향은 [Project Management Direction](./project-management.md)에 정리한다.

외부 공개 프로젝트 리뷰도 지원한다. 사용자는 자기 프로젝트뿐 아니라 참고할 만한 공개 프로젝트를 등록하고 리뷰할 수 있으며, 프로젝트 실제 소유자는 나중에 claim할 수 있다. 이 구조는 VibeReview를 주요 바이브 코딩 프로젝트 아카이브이자 북마크 공간으로 확장한다.
...
- 프로젝트 등록 직후 의미 있는 AI 첫 피드백이 생성된다.
