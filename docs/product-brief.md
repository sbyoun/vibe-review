...
외부 공개 프로젝트 리뷰도 지원한다. 사용자는 자기 프로젝트뿐 아니라 참고할 만한 공개 프로젝트를 등록하고 리뷰할 수 있으며, 프로젝트 실제 소유자는 나중에 claim해서 자기 프로필의 owned project로 가져갈 수 있다. 원 등록자는 `submittedBy`로 남고, 수정/삭제 권한은 클레임한 소유자에게 이동한다. 이 구조는 vibearchive를 주요 바이브 코딩 프로젝트 아카이브이자 북마크 공간으로 확장한다.
...
외부 공개 프로젝트는 기본적으로 claim 전 상태를 표시한다. 실제 소유자가 claim하면 프로젝트는 소유자 프로필로 이동하고, 원 등록자는 submittedBy 기록으로 남는다. 소유자 요청이 있으면 삭제 또는 비공개 처리한다.
