...
2. `GET /api/mcp/auth/check`로 인증과 사용자 매핑을 확인한다.
3. `GET /api/mcp/schema`로 최신 API 계약을 읽는다.
4. `GET /api/mcp/projects`로 기존 프로젝트를 조회한다.
5. 같은 프로젝트가 이미 있으면 새로 만들지 말고 해당 `project.id`를 사용한다.
6. 없으면 `POST /api/mcp/projects`로 프로젝트를 생성한다.
7. 사용자가 피드백 요청을 원했거나 자동화 규칙상 필요하면
   `POST /api/mcp/projects/{projectId}/feedback-requests`를 호출한다.
8. 이후 작업 루프에서는 `GET /api/mcp/feedback?projectId={projectId}&limit=50`으로
